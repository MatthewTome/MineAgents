...
export class ResourceProtocol
{
    ...
    private offers: Map<string, ResourceOffer> = new Map();
    ...
    createRequest(item: string, count: number, urgent: boolean = false): ResourceRequest
    {
    ...
    formatRequestMessage(request: ResourceRequest): string
    {
    ...
    parseRequestFromChat(message: string, senderName: string): ResourceRequest | null
    {
        ...
        if (teamNeedsMatch)
        {
            const request: ResourceRequest = {
                ...
                requesterRole: teamNeedsMatch[2] as AgentRole,
    ...
    parseOfferFromChat(message: string, senderName: string): ResourceOffer | null
    {
    ...
    getMyPendingRequests(): ResourceRequest[]
    {
        return Array.from(this.requests.values()).filter(
            r => r.requester === this.myName && r.status === "pending"
        );
    }

    getAvailableOffers(): ResourceOffer[]
    {
    ...
    canIHelpWith(request: ResourceRequest): boolean
    {

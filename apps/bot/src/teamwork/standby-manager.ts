...
import type { TeamPlanFile } from "./team-plan.js";

export type StandbyState = "active" | "standby" | "responding";
...
{
    listenPatterns: RegExp[];
    proactiveChecks: string[];
    standbyMessage: string;
    canRespondTo: AgentRole[];
}
...
const ROLE_STANDBY_BEHAVIORS: Record<AgentRole, StandbyBehavior> =
{
    supervisor: {
        listenPatterns: [
            /\[problem\]/i,
...
export interface ParsedResourceRequest
{
    requester: string;
...
{
    private state: StandbyState = "standby";
    private role: AgentRole;
    private botName: string;
    private lastAnnouncementTime: number = 0;
    private announcementCooldownMs: number = 60000;
    private hasAnnouncedStandby: boolean = false;
    private awaitingTeamPlan: boolean = false;

    constructor(role: AgentRole, botName: string)
    {
        this.role = role;
    ...
    enterStandby(bot: Bot, reason: string): void
    {
    ...
    exitStandby(reason?: string): void
    {
        if (this.state !== "standby")
        {
            return;
    ...
    shouldRespondToMessage(message: string, senderName: string | undefined, senderRole?: AgentRole): boolean
    {
        if (this.state !== "standby")
        {
            return false;
    ...
    parseResourceRequest(message: string): ParsedResourceRequest | null
    {
    ...
    buildResponseGoal(request: ParsedResourceRequest): string
    {
    ...
    resetAnnouncementFlag(): void
    {
        this.hasAnnouncedStandby = false;
    }


    isWithinBuildSite(bot: Bot, teamPlan: TeamPlanFile | null): boolean
    {
        if (!teamPlan?.sharedOrigin) { return false; }
    ...
    calculateSafeStandbyPosition(bot: Bot, teamPlan: TeamPlanFile | null): Vec3 | null
    {
        if (!teamPlan?.sharedOrigin) { return null; }
    ...
    isMoveRequest(message: string, senderName: string | undefined): boolean
    {
        if (!message || !senderName) { return false; }
        if (senderName.toLowerCase() === this.botName.toLowerCase()) { return false; }

        if (!MOVE_REQUEST_PATTERN.test(message)) { return false; }
    ...
    parseMoveRequest(message: string): { requester: string; targetPosition?: Vec3 } | null
    {
    ...
    buildMoveAwayGoal(bot: Bot, teamPlan: TeamPlanFile | null): string
    {
...
export function getStandbyBehavior(role: AgentRole): StandbyBehavior
{
    return ROLE_STANDBY_BEHAVIORS[role];
}

export function canRolePlanIndependently(role: AgentRole): boolean
{
    return PLANNER_ROLES.includes(role);
}

export function isSpecialistRole(role: AgentRole): boolean
{
    return SPECIALIST_ROLES.includes(role);
}

export function isMoveRequestMessage(message: string): boolean
{
...
): Vec3
{
    ...
    return new Vec3(
        origin.x + normX * minDistance,
        botPosition.y,
        origin.z + normZ * minDistance
    );
}
...
export function buildPlannerPrompt(options: HuggingFacePlannerOptions, request: PlanRequest, knowledge: string[] = []): string
{
    ...
    return [
        ...
        "Return JSON with fields intent (short string), goal_achieved (boolean), and steps (array of {id, action, params?, description?}).",
        ...
        "2. Use only high-level actions (mine, craft, smelt, build, place, loot, etc). Runtime handles movement/pathing/ordering.",
        ...
        "4. Include params.count with the exact quantity of needed items/blocks (especially for mine, loot, craft, smelt, drop, give).",
        ...
        "6. Include params.count with the exact quantity of needed items/blocks (especially for mine, loot, craft, smelt, drop, give).",
        planningModeRules,

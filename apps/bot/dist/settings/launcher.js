...
async function main() {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const botConfig = {
            ...
            agent: { role, mentor: { mode: "none", adviceCooldownMs: 15000, requestCooldownMs: 30000 } },
            features: { ragEnabled: true, narrationEnabled: true, safetyEnabled: true }
        };
        fs.writeFileSync(configPath, dumpYaml(botConfig));
        agents.push({ name, role, configPath });
    }
    rl.close();
    console.log("\n[Launcher] Building project...");
    try {
    ...
    agents.forEach((agent, idx) => {
        ...
        exec(command, (error) => {
            if (error)
                console.error(`[Launcher] Error spawning ${agent.name}:`, error);
        });
    });
}
main();
